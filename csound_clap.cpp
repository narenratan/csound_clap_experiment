/*
    This is the code from part 2 of nakst's CLAP tutorial, modified to use
    Csound to generate the sound

    (original code at https://nakst.gitlab.io/tutorial/clap-part-2.html)
*/
#include <string.h>
#include <stdlib.h>
#include <stdio.h>
		...
		if (event->type == CLAP_EVENT_NOTE_ON || event->type == CLAP_EVENT_NOTE_OFF || event->type == CLAP_EVENT_NOTE_CHOKE) {
			...
			if (event->type == CLAP_EVENT_NOTE_ON) {
                ...
                MYFLT p[5] = {1,
                              ...
                              3,
                              (MYFLT)noteEvent->key,
                              (MYFLT)noteEvent->note_id};
                plugin->cs->ScoreEvent('i', p, 5);
                plugin->cs->SetControlChannel(("Volume" + std::to_string(noteEvent->note_id)).c_str(), plugin->parameters[P_VOLUME]);
			}
            ...
            plugin->cs->SetControlChannel("Volume", plugin->parameters[i]);
			...
			for (int i = 0; i < plugin->voices.Length(); i++) {
				...
				if ((modEvent->key == -1 || voice->key == modEvent->key)
                    ...
                    plugin->cs->SetControlChannel(("Volume" + std::to_string(modEvent->note_id)).c_str(), plugin->parameters[P_VOLUME] + modEvent->amount);
					break;
				}
			}
		}
...
static void PluginRenderAudio(MyPlugin *plugin, uint32_t start, uint32_t end, float *outputL, float *outputR) {
    int pos, ksmps = plugin->cs->GetKsmps();
    MYFLT scale = plugin->cs->Get0dBFS();
    MYFLT *spout = plugin->cs->GetSpout();

    int frames = ksmps;
    int result = 0;
    for (uint32_t i = start; i < end; i++, frames++)
    {
        if (frames == ksmps)
        {
            result = plugin->cs->PerformKsmps();
            frames = 0;
        }
        if (!result)
        {
            pos = frames * 2;
            outputL[i] = spout[pos] / scale;
            outputR[i] = spout[pos + 1] / scale;
        }
        else
        {
            outputL[i] = 0;
...
static const clap_plugin_t pluginClass = {
	...
	.init = [] (const clap_plugin *_plugin) -> bool {
        ...
        plugin->cs = new Csound();
        plugin->cs->SetHostImplementedAudioIO(1, 0);
        int result = plugin->cs->Compile("/home/naren/repos/csound_clap/synth.csd");
        assert(result == 0);
		...
		for (uint32_t i = 0; i < P_COUNT; i++) {
            ...
            plugin->cs->SetControlChannel(information.name, plugin->parameters[i]);
		}
